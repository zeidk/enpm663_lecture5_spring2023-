...
void SensorCamera::LeftBinsCameraCallback(const ariac_msgs::msg::AdvancedLogicalCameraImage::ConstSharedPtr msg)
{
    ...
    for (auto part_pose : part_poses)
    {
        ...
        RCLCPP_INFO_STREAM(this->get_logger(), "Part type: " << part.type);
        RCLCPP_INFO_STREAM(this->get_logger(), "Part color: " << part.color);
        auto pose = part_pose.pose;
        RCLCPP_INFO(this->get_logger(), "Position: %f, %f, %f", pose.position.x, pose.position.y, pose.position.z);
        RCLCPP_INFO(this->get_logger(), "Orientation: %f, %f, %f, %f", pose.orientation.x, pose.orientation.y, pose.orientation.z, pose.orientation.w);
    }
}
// ================================
